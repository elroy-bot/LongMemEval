...
def generate_random_dates_before(input_date_str, n, days=30):
    ...
    random_dates = [one_month_before + timedelta(days=random.randint(0, (input_date - one_month_before).days)) for _ in range(n)]
    # Sort the dates
    random_dates.sort()
...
def generate_random_dates_after(input_date_str, n, days=30):
    ...
    # Generate n random dates within the range
    random_dates = [input_date + timedelta(days=random.randint(1, (one_month_after - input_date).days)) for _ in range(n)]
    # Sort the dates
    random_dates.sort()
    return [date.strftime("%Y/%m/%d") for date in random_dates]
...
def generate_random_dates_in_range(start_date_str, end_date_str, n):
    ...
    # Generate n random dates within the range
    random_dates = [start_date + timedelta(days=random.randint(0, delta_days)) for _ in range(n)]
    # Sort the dates
    random_dates.sort()
    return [date.strftime("%Y/%m/%d") for date in random_dates]
...
if enforce_json_length is not None:
    ...
    out_file = "{}_{}_{}-{}haysess_{}_enflen{}.json".format(
        prefix, task, min_n_haystack_filler, max_n_haystack_filler, suffix, enforce_json_length
    )
else:
...
qid2attid = {
    x["question_id"]: bgid_to_attribute[x["background_id"]] if x["background_id"] in bgid_to_attribute else x["background_id"]
...
print(
    "Haystack source: {} user, {} sharegpt, {} ultrachat".format(
        len(haystack_source_sessions_user_all), len(haystack_source_sessions_sharegpt), len(haystack_source_sessions_ultrachat)
    )
)
...
if task != "assistant_previnfo":
    ...
    for x in question_db:
        if x["question_type"] != task:
            continue
        if not [
            y for y in x["sessions"] if "human_valid_label" in sess_idx[y["session_id"]] and sess_idx[y["session_id"]]["human_valid_label"]
        ]:
            continue
...
for question_entry in tqdm(questions):
    ...
    haystack = [
        {"session_id": x["session_id"], "session": [{"role": y["role"], "content": y["content"]} for y in x["session"]]} for x in haystack
    ]
    ...
    if task in ["two_hop", "multi_session_synthesis", "single_hop", "implicit_preference", "implicit_preference_v2", "assistant_previnfo"]:
        ...
        question_date = (
            question_entry["question_content"]["question_date"] if "question_date" in question_entry["question_content"] else dates[-1]
        )
    ...
    if enforce_json_length:
        while len(tokenizer.encode(json.dumps(out_entry["haystack_sessions"]))) > enforce_json_length:
            ...
            out_entry["haystack_session_ids"] = (
                out_entry["haystack_session_ids"][:remove_idx] + out_entry["haystack_session_ids"][remove_idx + 1 :]
            )
...
print(
    "Max token count {}; Min token count {}; Mean token count {}".format(
        max(out_token_counts), min(out_token_counts), round(sum(out_token_counts) / len(out_token_counts), 2)
    )
)
